...
transaction(submittingNFTID: UInt64, playerTwoAddr: Address, matchTimeLimitInMinutes: UInt) {
    ...
    let gamePlayerTwoPublicRef: &{RockPaperScissorsGame.GamePlayerPublic}
    ...
    prepare(acct: AuthAccount) {
        ...
        self.gamePlayerTwoPublicRef = playerTwoAccount.getCapability<&{RockPaperScissorsGame.GamePlayerPublic}>(
                RockPaperScissorsGame.GamePlayerPublicPath
            ).borrow()
            ?? panic("GamePlayerPublic not accessible at address ".concat(playerTwoAddr.toString()))
        
        let receiverCap = acct.getCapability<&GamePieceNFT.Collection{NonFungibleToken.Receiver}>(
    ...
    execute {
        ...
        self.gamePlayerRef.addPlayerToMatch(matchID: self.newMatchID, gamePlayerRef: self.gamePlayerTwoPublicRef)
    }
}
